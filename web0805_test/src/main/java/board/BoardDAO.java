...
public class BoardDAO {
	...
	private String BOARD_MY_LIST = "select * from board where writer = ?";
	private String BOARD_RANK_LIST = "select * from board order by hits desc";
	...
	public void hitsUp(int num) {
		...
		try {
			stmt = conn.prepareStatement(HITS_UP);
			stmt.setInt(1, num);
			stmt.executeUpdate();
		} catch (SQLException e) {
			e.printStackTrace();
	...
	//나의 게시판 목록 조회
	public List<BoardDTO> getBoardMyList(String writer) {
		List<BoardDTO> list = new ArrayList<>();
			
		conn = JDBCUtil.getConnection();
		try {
			stmt = conn.prepareStatement(BOARD_MY_LIST);
	...
	//나의 게시판 목록 조회
	public List<BoardDTO> getBoardRankList() {
		List<BoardDTO> list = new ArrayList<>();
					
		conn = JDBCUtil.getConnection();
		try {
			stmt = conn.prepareStatement(BOARD_RANK_LIST);
			rs = stmt.executeQuery();
			while (rs.next()) {
				BoardDTO dto = new BoardDTO(rs.getInt("num"), rs.getString("writer"),
						rs.getString("title"), rs.getString("content"),
